...
export default function FoundItemsBoard() {
...
const loadItems = useCallback(async () => {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('found-items-board-changes')
      .on(
        'postgres_changes',
  ...
  return (
    <div className="container">
      <div className="page-header">
        <div>
          <h1>Found items board</h1>
          <p>
            Items found around campus that staff have posted here.
            {!isAdmin && ' If one looks like yours, open Messages on it to coordinate pickup with the office.'}
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">No items posted to the board right now.</div>
      ) : (
        filtered.map((r, i) => (
